...
#     # Pending: certificates without any verification logs
#     pending_verifications = Certificate.query.outerjoin(VerificationLog).filter(VerificationLog.id.is_(None)).count()
...
#     pagination = Certificate.query.order_by(Certificate.issued_at.desc()).paginate(page=page, per_page=per_page, error_out=False)
